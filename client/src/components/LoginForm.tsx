...
import { Network, Mail, Lock, ArrowRight, ShieldCheck } from 'lucide-react';
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <label className="flex items-center space-x-2 cursor-pointer group">
            <input type="checkbox" className="w-4 h-4 rounded border-slate-700 bg-slate-900/50 text-primary focus:ring-primary focus:ring-offset-0" />
            <span className="text-xs text-slate-400 font-medium group-hover:text-slate-300 transition-colors">Remember me</span>
          </label>
          <button type="button" className="text-xs text-primary hover:text-primaryHover font-medium transition-colors">
            Forgot Password?
          </button>
        </div>
        ...
        <div className="pt-4 text-center border-t border-slate-800">
          <p className="text-sm text-slate-400">
            New to the network? <button type="button" onClick={onToggleForm} className="text-white hover:text-primary transition-colors font-medium">Create Account</button>
          </p>
        </div>
